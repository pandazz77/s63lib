...
void test_encrypt(){
    ...
    const std::string M_KEY = hex_to_string("3938373635");
    const std::string HW_ID = hex_to_string("3132333438");
    const std::string M_ID = hex_to_string("3031");

    const std::string CK1 = hex_to_string("C1CB518E9C");
    const std::string CK2 = hex_to_string("421571CC66");
    const std::string EXPDAY = "20251231";

    auto cellPermit = S63::createCellPermit(HW_ID,CK1,CK2,"US5OH10M",EXPDAY);

    std::filesystem::path s57path("dataset/s57/in/US5OH10M.000");
    std::ifstream inputFile(s57path, std::ios::binary);
    ...
    SimpleZip zip;
    std::string zipbuf;
    bool OK = zip.zip("US5OH10M.000",buffer,zipbuf);

    S63::encryptCell(zipbuf,CK1);

    std::ofstream outputFile("dataset/s63/out/US5OH10M.000", std::ios::binary);
    assert(outputFile.is_open());
    outputFile.write(zipbuf.data(), zipbuf.size());
    outputFile.close();


    std::ofstream permitFile("dataset/s63/out/PERMIT.txt",std::ios::binary);
    assert(permitFile.is_open());
    permitFile.write(cellPermit.data(),cellPermit.size());
    permitFile.close();

    cout << "ENCRYPTED" << endl;

    S63Client cl(HW_ID,M_KEY,M_ID);
    cl.installCellPermit(cellPermit);
    cl.decryptAndUnzipCell("dataset/s63/out/US5OH10M.000","dataset/s57/out/US5OH10M.000");
}
